...
function BaseUnit:Spawn(player: Player, spot: number, uniqueId: string)
    self.UniqueId = uniqueId
    self.Owner = player.UserId
    BattleUpdate:FireClient(player, "SpawnUnit", self, spot, false)
    ...
    part.Parent = workspace.Arena.Characters.Player
...
function BaseUnit:TakeDamage(amount: number)
...
function BaseUnit:IsAlive()
    return self.Health > 0
end

function BaseUnit:Attack(target)
